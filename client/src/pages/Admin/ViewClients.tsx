...
import { useGetClientsQuery, useDeleteClientMutation } from "@api/setupApi";
...
const ViewClients = () => {
  ...
  const [deleteBranch] = useDeleteClientMutation();

  // Function to handle delete action
  const handleDeleteClient = async () => {
    if (selectedClientId) {
      try {
        await deleteBranch(selectedClientId).unwrap();
        toast.success("Branch deleted successfully!");
      } catch (error) {
        toast.error("Failed to delete the branch.");
      } finally {
        setIsModalOpen(false);
        setSelectedClientId(null); // Reset after deletion
      }
    }
  };
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: { id: number } } }) => (
          <button
            onClick={() => {
              setSelectedClientId(`${row.original.id}`); // Set the selected branch ID
  ...
  const table = useReactTable({
    data: data?.data.clients || [], // Safely access data or provide empty array
    columns,
