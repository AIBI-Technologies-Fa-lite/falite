  ...
  const columns = useMemo(() => {
    const commonColumns = [
      ...
      { header: "Pincode", accessorKey: "pincode", enableSorting: true },
      {
        ...
        accessorKey: "tat",
        cell: ({ row }) => (
          <div className={`${row.original.tat.status === 1 ? "text-orange-500" : row.original.tat.status === 2 ? "text-red-500" : "text-green-500"}`}>
            {row.original.tat.time}
          </div>
        ),
        enableSorting: false
      },
      ...
      <div className="flex flex-col gap-2 mb-6 md:flex-row md:justify-between">
        <div className="flex items-center text-sm md:gap-4 md:justify-start md:text-base">
          ...
          >
            ...
            {[Role.ADMIN, Role.SUPERVISOR].includes(role) ? <option value="ofName">OF</option> : null}
