...
const ViewVerification = () => {
  ...
  const [submitResponse, { isLoading: submitingVerification }] =
    useSubmitVerificationMutation();
  ...
  const canSeeCREDocuments =
    role !== "OF" || (role === "OF" && verification.final === 0);
    ...
    <div className='flex flex-col w-full gap-6'>
      ...
      <div className='grid grid-cols-1 gap-2 md:grid-cols-3 md:gap-4'>
        ...
        <div className='flex flex-col justify-between gap-2 md:col-span-3 md:flex-row'>
          ...
          {role === "OF" &&
            verification.working == false &&
            verification.status != "PENDING" && (
              <button
                className='md:px-4 md:py-2 p-2 text-xs text-white transition-all duration-100 bg-purple-600 rounded-lg hover:bg-purple-400 w-fit hover:cursor-pointer disabled:bg-purple-300'
                disabled={isMarking}
                onClick={async () => {
        ...
        {/* Case Data Details */}
        {caseData?.employee?.firstName && (
          <CaseDetails label='CRE' value={caseData.employee.firstName} />
        )}
        {caseData?.applicantName && (
          <CaseDetails label='Applicant Name' value={caseData.applicantName} />
        )}
        {caseData?.coApplicantName && (
          <CaseDetails
            label='Co Applicant Name'
            value={caseData.coApplicantName}
          />
        )}
        {caseData?.businessName && (
          <CaseDetails label='Business Name' value={caseData.businessName} />
        )}
        {caseData?.product && (
          <CaseDetails label='Product' value={caseData.product} />
        )}
        {caseData?.clientName && (
          <CaseDetails label='Client' value={caseData.clientName} />
        )}

        <CaseDetails
          label='Verification Type'
          value={verification.verificationType.name}
        />
        <CaseDetails
          label='OF'
        ...
        {verification.documents.length > 0 && (
          <ShowDocuments
            canSeeCREDocuments={canSeeCREDocuments}
      ...
      {billingModal && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'>
          <div className='p-6 bg-white rounded-lg shadow-lg md:w-[40%] w-[90%]'>
            ...
            <form
              className=''
              onSubmit={handleSubmit(markBillableVerification)}
            >
              <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 placeholder:text-gray-400'>
                <div className='flex flex-col gap-2 md:col-span-1'>
                  ...
                  <select
                    className='p-2 border-gray-500 rounded-lg border-2'
                    {...register("ofBillable", { required: true })}
                    defaultValue=''
                  >
                ...
                <div className='flex flex-col gap-2 md:col-span-1'>
                  <label>
                    Billable To Client{" "}
                    {errors.clientBillable && (
                      <span className='text-red-500'>*</span>
                    )}
                  </label>
                  <select
                    className='p-2 border-gray-500 rounded-lg border-2'
                    {...register("clientBillable", { required: true })}
                    defaultValue=''
                  >
                    <option value={1}>Billable</option>
                    <option value={0}>Not Billable</option>
